...
async function main() {
	...
	try {
		...
		try {
			...
			console.warn(
				"\x1b[31mFailed to pull core repository. It might not be a git repo or has local changes.\x1b[0m",
			);
		}
		...
		for (const sdk of sdks) {
			...
			if (existsSync(sdkPath)) {
				...
				try {
					...
					console.warn(
						`\x1b[31mFailed to update ${sdk.name}. Skipping...\x1b[0m`,
					);
				}
			} else {
		...
		try {
			...
			console.warn(
				"\x1b[31mFailed to update GeoIP database. Continuing...\x1b[0m",
			);
		}
		...
		console.log(`\n\x1b[32m${"=".repeat(60)}`);
		console.log(
			"   SUCCESS: MetaBun Framework, SDKs, and GeoIP are up to date!",
		);
		console.log(`${"=".repeat(60)}\x1b[0m`);
	} catch (error: any) {
