...
describe("Eklenti Bağlamı (PluginContext) Birim Testleri", () => {
  ...
  beforeAll(() => {
    ...
    writeFileSync(join(transDir, "en.json"), JSON.stringify({
      "welcome": "Welcome {0} to the server!"
    }));
    writeFileSync(join(transDir, "tr.json"), JSON.stringify({
  ...
  it("Çeviri dosyalarını yükleyip oyuncunun dil seçeneğine göre yerelleştirilmiş mesajlar döndürmelidir", () => {
    const playerEn = new Player(bridge, adminManager, banManager, 1, "Alice", "STEAM_A", 101);
    playerEn.SetLanguage("en");
    playerManager.AddPlayer(playerEn);

    const playerTr = new Player(bridge, adminManager, banManager, 2, "Ahmet", "STEAM_B", 102);
  ...
  it("Olay dinleyicilerini (event hooks) dinamik olarak kaydedebilmeli ve eklenti kaldırıldığında bunları temizleyebilmelidir", async () => {
    ...
    expect(lastAction).toEqual({ action: "hook_event", event: "PlayerSpawned" });
    ...
    expect(lastAction).toEqual({ action: "unhook_event", event: "PlayerSpawned" });
  });
});
