...
import type { PlayerChatEvent } from "../../src/ts/shared/types/events";
import CoreCommands from "../../plugins/core-commands";
import PlayerCommands from "../../plugins/player-commands";
import AdminChat from "../../plugins/admin-chat";
import ReservedSlots from "../../plugins/reserved-slots";
...
describe("Kapsamlı Gerçek Zamanlı Sistem Entegrasyon Testi", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(join(translationsDir, "en.json"), JSON.stringify({
  ...
  it("Oyuncu istatistiklerini belirli periyotlarla veritabanına otomatik kaydetmelidir (checkpoint)", () => {
    const player = new Player(bridge, adminManager, banManager, 1, "StatsPlayer", "STEAM_0:0:99", 10);
  ...
  it("Oyuncu envanterini ve sahip olunan silahların takibini hatasız yapabilmelidir", () => {
    const player = new Player(bridge, adminManager, banManager, 1, "WeaponGuy", "STEAM_0:0:101", 11);
  ...
  it("sm_ping komutunu işleyebilmeli ve tüm oyunculara Pong mesajı yayınlayabilmelidir", async () => {
    ...
    const p = new Player(bridge, adminManager, banManager, 1, "Alice", "STEAM_A", 101);
  ...
  it("Raund sıfırlama (restart) yetkisini sadece admin veya sunucu konsolu ile sınırlandırmalıdır", async () => {
    ...
    const normal = new Player(bridge, adminManager, banManager, 1, "Player", "STEAM_0:0:1", 1);
    const admin = new Player(bridge, adminManager, banManager, 2, "Admin", "STEAM_0:0:2", 2);
  ...
  it("Oyuncu indeksine veya ismine göre kimlik bilgilerini (whois) sorgulayabilmelidir", async () => {
    ...
    const target = new Player(bridge, adminManager, banManager, 3, "TargetUser", "STEAM_0:0:33", 3);
    playerManager.AddPlayer(target);

    // Add caller player at index 1 to receive replies
    const caller = new Player(bridge, adminManager, banManager, 1, "Unknown", "STEAM_0:0:1", 1);
    ...
    expect(lastSay).toBe("Oyuncu: TargetUser | SteamID: STEAM_0:0:33 | Index: 3");
  ...
  it("Admin yetkisine göre tek bir oyuncuyu tokatlayabilmeli (slap) veya tüm oyuncuları tokatlayabilmelidir (slapall)", async () => {
    ...
    const admin = new Player(bridge, adminManager, banManager, 1, "AdminGuy", "STEAM_0:0:1", 1);
    const player1 = new Player(bridge, adminManager, banManager, 2, "PlayerOne", "STEAM_0:0:2", 2);
    const player2 = new Player(bridge, adminManager, banManager, 3, "PlayerTwo", "STEAM_0:0:3", 3);
    playerManager.AddPlayer(admin);
    ...
    expect(sentActions).toContainEqual({ action: "slap", client: "2", damage: "15" });
    expect(lastSay).toBe("Admin AdminGuy, PlayerOne oyuncusunu 15 hasar ile tokatladı.");
  ...
  it("Yerelleştirilmiş çeviri formatını kullanarak oyuncu istatistiklerini gösterebilmelidir", async () => {
    ...
    const player = new Player(bridge, adminManager, banManager, 5, "StatsUser", "STEAM_0:0:5", 5);
    ...
    expect(lastSay).toBe("(To StatsUser) Player: StatsUser | Kills: 25 | Deaths: 10");
  });
  ...
  it("sm_menu gösterimini yönetebilmeli ve seçim yapıldığında ilgili callback fonksiyonunu tetiklemelidir", async () => {
    ...
    const admin = new Player(bridge, adminManager, banManager, 1, "AdminMenuUser", "STEAM_0:0:7", 7);
    playerManager.AddPlayer(admin);
  ...
  it("Oyuncuya ait tüm olay dinleyicilerini (can, takım, ölüm, silah değişikliği) desteklemelidir", () => {
    const player = new Player(bridge, adminManager, banManager, 1, "EventPlayer", "STEAM_0:0:400", 400);
    playerManager.AddPlayer(player);
    ...
    player.on("HealthChange", (val) => { healthVal = val; });
    player.on("TeamChange", (val) => { teamVal = val; });
    player.on("WeaponChange", (val) => { weaponVal = val; });
  ...
  it("Tüm oyuncu komut eylemlerinin Metamod köprüsüne doğru veri paketleri (payload) göndermesini sağlamalıdır", () => {
    const player = new Player(bridge, adminManager, banManager, 10, "ActionPlayer", "STEAM_0:0:500", 500);
    playerManager.AddPlayer(player);
    ...
    expect(sentActions.pop()).toEqual({ action: "say", text: "(To ActionPlayer) Test message" });

    // 2. Kick with reason
    player.Kick("Toxic");
    expect(sentActions.pop()).toEqual({ action: "kick", client: "500", reason: "Toxic" });

    // 3. Kick without reason
    player.Kick();
    expect(sentActions.pop()).toEqual({ action: "kick", client: "500", reason: "Kicked by admin" });

    // 4. Slap
    player.Slap(30);
    expect(sentActions.pop()).toEqual({ action: "slap", client: "10", damage: "30" });

    // 5. Teleport
    player.Teleport(123.4, -567.8, 90.1);
    expect(sentActions.pop()).toEqual({ action: "teleport", client: "10", x: "123.4", y: "-567.8", z: "90.1" });

    // 6. SetTeam
    player.SetTeam(Team.CT);
    expect(sentActions.pop()).toEqual({ action: "set_team", client: "10", team: "3" });
    ...
    expect(sentActions.pop()).toEqual({ action: "set_gravity", client: "10", gravity: "0.5" });

    // 9. SetMoveType
    player.SetMoveType(2);
    expect(sentActions.pop()).toEqual({ action: "set_movetype", client: "10", movetype: "2" });

    // 10. SetHealth
    player.SetHealth(150);
    expect(sentActions.pop()).toEqual({ action: "set_health", client: "10", health: "150" });

    // 11. SetModel
    player.SetModel("models/player/custom.mdl");
    expect(sentActions.pop()).toEqual({ action: "set_model", client: "10", model: "models/player/custom.mdl" });

    // 12. SetRenderColor
    player.SetRenderColor(255, 0, 0, 128);
    expect(sentActions.pop()).toEqual({ action: "set_render_color", client: "10", r: "255", g: "0", b: "0", a: "128" });

    // 13. EmitSound
    player.EmitSound("ambient/alarms/alarm1.wav");
    expect(sentActions.pop()).toEqual({ action: "play_sound", client: "10", sound: "ambient/alarms/alarm1.wav", all: "false" });
  });
  ...
  it("PluginContext entegrasyonu kapsamındaki tüm özellikleri (aksiyonlar, izinler, mesajlar, istemci bilgileri) desteklemelidir", async () => {
    ...
    writeFileSync(join(translationsDir, "tr.json"), JSON.stringify({
    ...
    const englishPlayer = new Player(bridge, adminManager, banManager, 1, "EnglishUser", "STEAM_0:0:11", 11);
    ...
    const turkishPlayer = new Player(bridge, adminManager, banManager, 2, "TurkishUser", "STEAM_0:0:22", 22);
    ...
    expect(sentActions.pop()).toEqual({ action: "say", text: "(To EnglishUser) Hello English" });

    context.PrintToChat(0, "Broadcast message");
    expect(sentActions.pop()).toEqual({ action: "say", text: "Broadcast message" });
    ...
    expect(sentActions.pop()).toEqual({ action: "say", text: "(To EnglishUser) Replying" });

    // B. Translation formatting
    context.TPrintToChat(1, "stats_info", "EnglishUser", 5, 2);
    expect(sentActions.pop()).toEqual({ action: "say", text: "(To EnglishUser) Player: EnglishUser | Kills: 5 | Deaths: 2" });

    context.TPrintToChat(2, "stats_info", "TurkishUser", 8, 4);
    expect(sentActions.pop()).toEqual({ action: "say", text: "(To TurkishUser) Oyuncu: TurkishUser | Leş: 8 | Ölüm: 4" });
    ...
    expect(sentActions.pop()).toEqual({ action: "slap", client: "2", damage: "10" });

    context.TeleportEntity(1, 10.0, 20.0, 30.0);
    expect(sentActions.pop()).toEqual({ action: "teleport", client: "1", x: "10", y: "20", z: "30" });

    context.ChangeClientTeam(1, Team.Terrorist);
    expect(sentActions.pop()).toEqual({ action: "set_team", client: "1", team: "2" });

    context.RespawnPlayer(2);
    expect(sentActions.pop()).toEqual({ action: "respawn", client: "2" });

    context.GivePlayerItem(2, "weapon_m4a1");
    expect(sentActions.pop()).toEqual({ action: "give_item", client: "2", item: "weapon_m4a1" });

    context.RemovePlayerItem(2, "weapon_ak47");
    expect(sentActions.pop()).toEqual({ action: "remove_item", client: "2", item: "weapon_ak47" });

    context.SetWeaponAmmo(2, "weapon_m4a1", 90);
    expect(sentActions.pop()).toEqual({ action: "set_ammo", client: "2", weapon: "weapon_m4a1", ammo: "90" });

    context.SetEntityGravity(1, 0.9);
    expect(sentActions.pop()).toEqual({ action: "set_gravity", client: "1", gravity: "0.9" });

    context.SetEntityMoveType(1, 4);
    expect(sentActions.pop()).toEqual({ action: "set_movetype", client: "1", movetype: "4" });

    context.SetEntityHealth(1, 95);
    expect(sentActions.pop()).toEqual({ action: "set_health", client: "1", health: "95" });

    context.SetEntityModel(1, "models/gign.mdl");
    expect(sentActions.pop()).toEqual({ action: "set_model", client: "1", model: "models/gign.mdl" });

    context.SetEntityRenderColor(1, 0, 255, 0, 255);
    expect(sentActions.pop()).toEqual({ action: "set_render_color", client: "1", r: "0", g: "255", b: "0", a: "255" });

    context.EmitSoundToClient(1, "weapons/c4/c4_beep.wav");
    expect(sentActions.pop()).toEqual({ action: "play_sound", client: "1", sound: "weapons/c4/c4_beep.wav", all: "false" });

    context.EmitSoundToAll("weapons/explode3.wav");
    expect(sentActions.pop()).toEqual({ action: "play_sound", sound: "weapons/explode3.wav", all: "true" });

    context.KickClient(2, "Cheating");
    expect(sentActions.pop()).toEqual({ action: "kick", client: "22", reason: "Cheating" });

    context.BanClient("STEAM_0:0:666", "Wallhack", "STEAM_0:0:11", 1440);
    expect(sentActions.pop()).toEqual({ action: "ban", steamid: "STEAM_0:0:666", duration: "1440", reason: "Wallhack" });

    context.RemoveBan("STEAM_0:0:666");
    expect(sentActions.pop()).toEqual({ action: "unban", steamid: "STEAM_0:0:666" });
  ...
  it("SQL tabanlı ban kontrollerini uygulayarak yasaklı oyuncuların bağlantısını reddetmelidir", async () => {
    ...
    await banManager.BanClient(steamId, "Abusive behavior", "STEAM_0:0:100", 60); // 60 mins
    expect(await banManager.CheckBan(steamId)).toBe(true);
    expect(await banManager.CheckBan(normalSteamId)).toBe(false);

    // 3. Connect players to PlayerManager and check connection flow
    const normalPlayer = new Player(bridge, adminManager, banManager, 10, "FriendlyUser", normalSteamId, 10);
    const toxicPlayer = new Player(bridge, adminManager, banManager, 11, "ToxicUser", steamId, 11);
  ...
  it("Oyuncunun mute, gag ve silence durumlarını takip edebilmeli ve gag durumunda mesaj göndermesini engellemelidir", () => {
    const player = new Player(bridge, adminManager, banManager, 1, "MutedPlayer", "STEAM_0:0:600", 600);
    playerManager.AddPlayer(player);
    ...
    expect(lastSay).toBe("(To MutedPlayer) You cannot send messages while your chat is gagged.");
  });
  ...
  it("Adminler arası hedef alma işlemlerinde bağışıklık derecelerini (immunity) doğrulamalıdır", () => {
    ...
    const rootAdmin = new Player(bridge, adminManager, banManager, 1, "Root", "STEAM_ROOT", 101);
    const vipAdmin = new Player(bridge, adminManager, banManager, 2, "VIP", "STEAM_VIP", 102);
    const regularPlayer = new Player(bridge, adminManager, banManager, 3, "Player", "STEAM_PLAYER", 103);
  ...
  it("Kafadan vuruş, hasar ve MVP gibi gelişmiş istatistiklerin takibini ve kaydedilmesini desteklemelidir", () => {
    const player = new Player(bridge, adminManager, banManager, 1, "StatsPro", "STEAM_STATS", 700);
    playerManager.AddPlayer(player);
  ...
  it("AFK kalan oyuncuları tespit edip otomatik olarak spec (izleyici) takımına taşımalıdır", () => {
    const player = new Player(bridge, adminManager, banManager, 1, "AFKPlayer", "STEAM_AFK", 800);
    player.UpdateTeam(Team.Terrorist);
    playerManager.AddPlayer(player);
    ...
    expect(sentActions).toContainEqual({ action: "set_team", client: "1", team: "1" });
    expect(lastSay).toBe("(To AFKPlayer) You have been moved to spectator for being AFK.");
  ...
  it("VIP slot hakkını doğrulamalı ve sunucu doluyken normal oyuncuyu atarak VIP girişine izin vermelidir", () => {
    ...
    const p1 = new Player(bridge, adminManager, banManager, 1, "Normal1", "STEAM_P1", 901);
    const p2 = new Player(bridge, adminManager, banManager, 2, "Normal2", "STEAM_P2", 902);
  ...
  it("Callback fonksiyonları yardımıyla oylama sistemini başlatmayı ve oy dağılımını toplamayı desteklemelidir", async () => {
    ...
    const p1 = new Player(bridge, adminManager, banManager, 1, "PlayerOne", "STEAM_P1", 901);
    const p2 = new Player(bridge, adminManager, banManager, 2, "PlayerTwo", "STEAM_P2", 902);
    playerManager.AddPlayer(p1);
    playerManager.AddPlayer(p2);
    ...
    const votePromise = new Promise<Record<string, number>>((resolve) => {
      context.CreateVote("Gelecek harita ne olsun?", ["de_dust2", "de_inferno"], (results) => {
        resolve(results);
      }, 50); // 50ms vote duration
    });

    // Simulate voting
    pluginManager.emit("MenuSelect", { client: 1, menuId: lastMenuId, info: "de_dust2" });
    pluginManager.emit("MenuSelect", { client: 2, menuId: lastMenuId, info: "de_inferno" });
  ...
  it("IP adresine göre GeoIP ülke sorgusunu, gelişmiş dosya loglayıcısını ve motor metriklerini desteklemelidir", () => {
    ...
    const player = new Player(bridge, adminManager, banManager, 1, "TurkishPro", "STEAM_GEO", 950);
