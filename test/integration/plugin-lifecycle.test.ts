...
describe("Eklenti Yaşam Döngüsü Entegrasyon Testleri", () => {
    ...
    `;
		...
		expect((pluginManager as any).loadedPlugins.has("functional.ts")).toBe(
			true,
		);
		...
		expect(logs.some((l) => l.includes("CustomEvent received: hello"))).toBe(
			true,
		);
    ...
    `;
		...
		const entry = (pluginManager as any).loadedPlugins.get(
			"auto_hook_plugin.ts",
		);
		const pluginInstance = entry.plugin;

		pluginManager.emit("CustomEvent", { event: "CustomEvent", payload: "ok" });
		expect(pluginInstance.receivedData).toEqual({
			event: "CustomEvent",
			payload: "ok",
		});
    ...
    `;
		...
		expect((pluginManager as any).loadedPlugins.has("my_folder_plugin")).toBe(
			true,
		);
    ...
    `;
		...
		expect(pluginInstance.eventData).toEqual({
			type: "command",
			client: 5,
			args: ["arg1", "arg2"],
		});
