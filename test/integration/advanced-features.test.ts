...
describe("İleri Düzey Özellikler Entegrasyon Testleri", () => {
    ...
    `;
		...
		let stateVal = pluginManager.GetPluginState(
			"StateTestPlugin",
			"load_counter",
			0,
		);
		...
		await pluginManager.LoadPlugin(fileName);
		stateVal = pluginManager.GetPluginState(
			"StateTestPlugin",
			"load_counter",
			0,
		);
	...
	it("Uzunluk ön ekli JSON protokolü kullanarak veri iletişimini sağlayabilmelidir", async () => {
		...
		// Setup client socket
		const clientPromise = new Promise<{ received: any[]; socket: any }>(
			(resolve, reject) => {
				const received: any[] = [];
	...
	it("Asenkron görevleri başarıyla çalıştırmalı ve senkron görevleri reddetmelidir", async () => {
		...
		expect(caughtError.toString()).toContain(
			"Action must be an asynchronous function",
		);
	});
});
