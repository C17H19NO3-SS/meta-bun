...
describe("Yeni Entegre Edilen SourceMod Özellikleri Test Grubu", () => {
  ...
  it("1. ConVar API -> ConVar oluşturabilmeli, güncelleyebilmeli ve change hook tetikleyebilmelidir", () => {
    pluginContextStore.run(context, () => {
      ...
      let oldVal = "";
      let newVal = "";
      cvar.AddChangeHook((cv, oldV, newV) => {
        hookCalled = true;
        oldVal = oldV;
        newVal = newV;
      });
  ...
  it("2. ClientPrefs Cookie API -> Cookie tanımlayabilmeli, set/get işlemlerini DB persist ile yapabilmelidir", () => {
    pluginContextStore.run(context, () => {
      const cookie = context.RegClientCookie("test_pref", "Test Cookie preference");
      ...
      const player = new Player(bridge, adminManager, banManager, 1, "CookieUser", "STEAM_0:0:555", 15);
  ...
  it("3. SQL_TQuery API -> Asenkron veritabanı sorgusu yapabilmelidir", async () => {
    ...
    const player = new Player(bridge, adminManager, banManager, 1, "SqlUser", "STEAM_0:0:777", 20);
    player.SetTotalStats(10, 2, 5);
    playerManager.AddPlayer(player);
    playerManager.Checkpoint(); // Flushes to DB

    await pluginContextStore.run(context, async () => {
      const rows = await context.SQL_TQuery("SELECT * FROM players WHERE steamid = ?", ["STEAM_0:0:777"]);
  ...
  it("4. HookEventPre API -> Olayları gerçekleşmeden önce engelleyebilmelidir", () => {
    pluginContextStore.run(context, () => {
      ...
      context.HookEventPre("PlayerSpawned", (data) => {
        preHookCalled = true;
        return Plugin_Handled;
      });

      // Register standard post hook
      pluginManager.on("PlayerSpawned", (data) => {
        postHookCalled = true;
      });

      // Emit the event via pluginManager
      const eventHandled = pluginManager.emit("PlayerSpawned", { client: 1, team: 2 });
  ...
  it("5. SDKHook API -> Düşük seviye SDK kancalarını çalıştırabilmelidir", () => {
    pluginContextStore.run(context, () => {
      ...
      context.SDKHook(1, SDKHookType.OnTakeDamage, (victim, attacker, damage) => {
        sdkHookCalled = true;
        damageAmount = damage;
        return Plugin_Handled;
      });
  ...
  it("6. MapChooser Plugin -> RTV ve Nominate oylaması akışını simüle edebilmelidir", async () => {
    ...
    // Mock players
    const p1 = new Player(bridge, adminManager, banManager, 1, "User1", "STEAM_0:0:1", 5);
    const p2 = new Player(bridge, adminManager, banManager, 2, "User2", "STEAM_0:0:2", 6);
    ...
    await pluginContextStore.run(mapChooserContext, async () => {
      ...
      mapChooserContext.CreateVote = (question, options, callback, duration) => {
        voteTriggered = true;
      ...
      pluginManager.emit("PlayerChat", { event: "PlayerChat", client: 1, text: "!rtv" });
      // RTV needs 60% of players, 2 players total -> 1 is 50%, needs 2
      expect(voteTriggered).toBe(false);

      // Trigger RTV for player 2
      pluginManager.emit("PlayerChat", { event: "PlayerChat", client: 2, text: "!rtv" });
  ...
  it("7. İzleyici, Bayrak ve Hız API'leri -> IsObserver, GetObserverTarget, GetEntityFlags ve Hız değerleri doğru şekilde işlenmelidir", () => {
    const player = new Player(bridge, adminManager, banManager, 1, "Speedy", "STEAM_0:0:888", 88);
    playerManager.AddPlayer(player);
    ...
    expect(lastAction).toEqual({ action: "set_velocity", client: "1", x: "100", y: "200", z: "300" });
  });
  ...
  it("8. Oylama İptali (CancelVote) -> Aktif oylamayı iptal edebilmeli ve state temizlemelidir", () => {
    ...
    const player = new Player(bridge, adminManager, banManager, 1, "Voter", "STEAM_0:0:999", 99);
    playerManager.AddPlayer(player);

    let voteCompleted = false;

    pluginContextStore.run(context, () => {
      context.CreateVote("Oylama Soru", ["A", "B"], (results) => {
        voteCompleted = true;
      }, 5000);
  ...
  it("9. SDKUnhook ve Köprü Bildirimleri -> SDK kancasını kaldırabilmeli ve C++ köprüsüne hook/unhook sinyallerini iletebilmelidir", () => {
    ...
    pluginContextStore.run(context, () => {
      const callback = (victim: number, attacker: number, damage: number) => {
        return 0;
      };

      // Hook
      context.SDKHook(1, SDKHookType.OnTakeDamage, callback);
      expect(sentActions).toContainEqual({ action: "hook_sdk", client: 1, type: SDKHookType.OnTakeDamage });

      // Unhook
      context.SDKUnhook(1, SDKHookType.OnTakeDamage, callback);
      expect(sentActions).toContainEqual({ action: "unhook_sdk", client: 1, type: SDKHookType.OnTakeDamage });
    });
  });
  ...
  it("10. ConVar Değişiklik Kancaları (Change Hooks) ve Köprü Senkronizasyonu", () => {
    ...
    pluginContextStore.run(context, () => {
      // Create ConVar
      const cvar = context.CreateConVar("sm_test_cvar", "10", "Test Description");
      expect(sentActions).toContainEqual({ action: "cvar_register", name: "sm_test_cvar", defaultValue: "10", description: "Test Description" });

      // Hook change
      let hookCalled = false;
      let hookOldVal = "";
      let hookNewVal = "";
      cvar.AddChangeHook((cv, oldVal, newVal) => {
        hookCalled = true;
        hookOldVal = oldVal;
        hookNewVal = newVal;
      });
      ...
      expect(sentActions).toContainEqual({ action: "cvar_set", name: "sm_test_cvar", value: "25" });
      expect(hookCalled).toBe(true);
      ...
      pluginManager.emit("ConVarChanged", { event: "ConVarChanged", name: "sm_test_cvar", value: "50" });
