import { readdirSync, existsSync, mkdirSync, watch, statSync, readFileSync, writeFileSync } from "node:fs";
import { EventEmitter } from "node:events";
import { resolve, join } from "node:path";
import { ReplySource } from "../shared/types/enums";
import type { Bridge } from "../network/bridge";
import type { IGameBridge, CommandCallback, Timer, IMenu, SDKHookType, ConVar, ClientCookie } from "../shared/types/bridge";
...
import { pluginContextStore, commandSourceStore } from "../shared/context-store";
...
export class PluginManager extends EventEmitter implements IGameBridge {
  ...
  private loadedPlugins: Map<string, { plugin: IPlugin; context: PluginContext }> = new Map();
  ...
  public preListeners: Map<string, Array<(data: GameEvent) => number>> = new Map();
  private sdkHooks: Map<number, Map<number, Array<(...args: unknown[]) => number>>> = new Map();
  private playerCommandTimestamps: Map<number, number> = new Map();
  private commandAliases: Map<string, string> = new Map();
  private sharedAPIs: Map<string, { pluginName: string; api: Record<string, unknown> }> = new Map();
  ...
  private async HandleMetaBunCommand(client: number, args: string[]): Promise<void> {
    ...
    if (subcommand === "list") {
      if (!existsSync(this.pluginsFolder)) {
        this.ReplyToCommand(client, "{Green}[Meta-Bun]{Default} Eklenti klasörü bulunamadı.");
        return;
      }
      ...
      for (const item of items) {
        ...
        if (item.isFile()) {
          ...
          const loaded = this.loadedPlugins.get(filename);
          if (loaded) {
            list.push(`{Green}[Loaded]{Default} ${loaded.plugin.name} v${loaded.plugin.version || "1.0.0"} - ${loaded.plugin.author || "Unknown"} (klasör: ${filename})`);
          } else {
    ...
    if (subcommand === "load") {
      this.ReplyToCommand(client, `{Green}[Meta-Bun]{Default} Eklenti yükleniyor: ${resolvedName}...`);
      await this.LoadPlugin(resolvedName);
      if (this.loadedPlugins.has(resolvedName)) {
        const loaded = this.loadedPlugins.get(resolvedName)!;
        this.ReplyToCommand(client, `{Green}[Meta-Bun]{Default} Eklenti başarıyla yüklendi: {LightBlue}${loaded.plugin.name}{Default} (${loaded.plugin.version})`);
      } else {
        this.ReplyToCommand(client, `{Green}[Meta-Bun]{Red} Hata:{Default} Eklenti yüklenemedi. Detaylar konsolda.`);
      }
      return;
    }
    ...
    if (subcommand === "unload") {
      if (!this.loadedPlugins.has(resolvedName)) {
        this.ReplyToCommand(client, `{Green}[Meta-Bun]{Red} Hata:{Default} Eklenti zaten yüklü değil: ${resolvedName}`);
        return;
      }
    ...
    if (subcommand === "reload") {
      if (!this.loadedPlugins.has(resolvedName)) {
        this.ReplyToCommand(client, `{Green}[Meta-Bun]{Red} Hata:{Default} Yeniden yüklenecek eklenti bulunamadı veya yüklü değil: ${resolvedName}`);
        return;
      }
      this.ReplyToCommand(client, `{Green}[Meta-Bun]{Default} Eklenti yeniden yükleniyor: ${resolvedName}...`);
      await this.LoadPlugin(resolvedName);
      if (this.loadedPlugins.has(resolvedName)) {
        const loaded = this.loadedPlugins.get(resolvedName)!;
        this.ReplyToCommand(client, `{Green}[Meta-Bun]{Default} Eklenti başarıyla yeniden yüklendi: {LightBlue}${loaded.plugin.name}{Default} (${loaded.plugin.version})`);
      } else {
        this.ReplyToCommand(client, `{Green}[Meta-Bun]{Red} Hata:{Default} Eklenti yeniden yüklenirken hata oluştu.`);
      }
      return;
    }
  }
  ...
  private HandlePluginsCommand(client: number, args: string[]): void {
  ...
  private HandleWhoCommand(client: number, args: string[]): void {
    const players = this.players.GetAll();
    this.ReplyToCommand(client, `{Gold}=== Aktif Oyuncular ve Yetkiler (${players.length}) ==={Default}`);
  ...
  private HandleAdminsCommand(client: number, args: string[]): void {
    ...
    try {
      ...
      for (const [id, entry] of Object.entries(admins)) {
        ...
        if (typeof entry === "string") {
          ...
          groups = (entry as any).groups ? ` (Grup: ${(entry as any).groups.join(", ")})` : "";
        }
  ...
  private HandleFsayCommand(client: number, args: string[]): void {
    ...
    const players = this.players.GetAll();
    const target = players.find(p => p.index.toString() === targetPattern || p.name.toLowerCase().includes(targetPattern.toLowerCase()));
  ...
  public SweepExpiredBans(): void {
    const pm = this.players as unknown as { db: { GetAllBans: () => Array<{ steamid: string; timestamp: number; duration: number }>; RemoveBan: (steamid: string) => void } };
    if (pm && pm.db) {
      try {
  ...
  private SetupCommandInterceptor() {
    this.HookEventPre("PlayerChat", (data: GameEvent) => {
      ...
      if (player && (player as unknown as { IsGagged: () => boolean }).IsGagged()) {
      ...
      if (player && !text.startsWith("!") && !text.startsWith("/")) {
        try {
          const chatColorsPath = resolve(process.cwd(), "configs", "admins", "chat_colors.json");
          if (existsSync(chatColorsPath)) {
            ...
            if (config.players && config.players[player.steamId]) {
      ...
      if (text.startsWith("!") || text.startsWith("/")) {
        ...
        if (commandWithTrigger) {
          ...
          if (cmdEntry) {
            ...
            if (process.env.NODE_ENV !== "test" && chatData.client !== 0 && now - lastCalled < 1000) {
              if (player) {
    ...
    this.HookEvent("ConsoleCommand", (data: GameEvent) => {
      const cmdData = data as unknown as { command: string; args: string[]; client: number };
  ...
  public RegConsoleCmd(command: string, callback: CommandCallback, flags?: string | null, description?: string | null): void {
  ...
  public CreateTimer(ms: number, callback: () => void, repeat?: boolean): Timer {
  ...
  public GetCmdReplySource(): ReplySource {
    return commandSourceStore.getStore() === "chat" ? ReplySource.Chat : ReplySource.Console;
  }
  ...
  public GetClientCount(inGameOnly: boolean = true): number {
  ...
  public BanClient(steamId: string, reason: string, adminSteamId: string, duration: number, ip: string = ""): void {
  ...
  public CreateMenu(title: string, callback: (client: number, info: string) => void): IMenu {
  ...
  public SetEntityRenderColor(client: number, r: number, g: number, b: number, a: number): void {
    this.players.Get(client)?.SetRenderColor(r, g, b, a);
  }

  public EmitSoundToClient(client: number, soundPath: string, volume?: number, channel?: number, pitch?: number): void {
    this.players.Get(client)?.EmitSound(soundPath, volume, channel, pitch);
  }

  public EmitSoundToAll(soundPath: string, volume?: number, channel?: number, pitch?: number): void {
    const payload: any = { action: "play_sound", sound: soundPath, all: "true" };
  ...
  // Permissions
  public CheckCommandAccess(client: number, command: string, flags: string): boolean {
  ...
  // Voting System
  public CreateVote(question: string, options: string[], callback: (results: Record<string, number>) => void, durationMs: number = 10000): void {
    if (this.activeVote) {
  ...
  public async LoadPlugin(nameOrPath: string) {
    ...
    try {
      ...
      if (typeof PluginClass === "function") {
        const constructor = PluginClass as unknown as { __commands?: Array<{ name: string; methodName: string; flags?: string; description?: string }>, __eventHooks?: Array<{ eventName: string; methodName: string }> };
          ...
          for (const cmd of constructor.__commands) {
            context.RegConsoleCmd(cmd.name, (client, args) => {
              return pluginContextStore.run(context, () => {
                return (pluginInstance as any)[cmd.methodName]?.(client, args);
              });
      ...
      const prototype = Object.getPrototypeOf(pluginInstance) as Record<string, unknown>;
      ...
      for (const key of allKeys) {
        if (key.startsWith("On") && key !== "OnLoad" && key !== "OnUnload") {
          const val = (pluginInstance as unknown as Record<string, unknown>)[key];
          if (typeof val === "function") {
            const eventName = key.substring(2); // e.g. "PlayerChat" from "OnPlayerChat"
            context.HookEvent(eventName, (data: GameEvent) => {
              pluginContextStore.run(context, () => (val as (data: GameEvent) => void).call(pluginInstance, data));
            });
  ...
  public GetPluginState<T>(pluginName: string, key: string, initialValue: T): T {
  ...
  // --- ConVar System ---
  public CreateConVar(name: string, defaultValue: string, description?: string): ConVar {
    if (this.convars.has(name)) {
      return this.convars.get(name)!;
    }
    const cvar = new ConVarImpl(name, defaultValue, description, (cname, cval) => {
  ...
  public async SQL_TQuery(sql: string, args: unknown[] = []): Promise<unknown[]> {
    const pm = this.players as unknown as { db: DatabaseManager };
    if (!pm.db) {
  ...
  public HookEventPre(event: string, callback: (data: GameEvent) => number): void {
    const eventStr = String(event);
  ...
  public UnhookEventPre(event: string, callback: (data: GameEvent) => number): void {
    const eventStr = String(event);
    ...
    if (preList && preList.length > 0) {
      ...
      for (const listener of preList) {
        try {
          ...
          if (result === 3 || result === 4) { // Plugin_Handled (3) / Plugin_Stop (4)
  ...
  // --- SDK Hooks ---
  public SDKHook(client: number, hookType: SDKHookType, callback: (...args: unknown[]) => number): void {
  ...
  public SDKUnhook(client: number, hookType: SDKHookType, callback: (...args: unknown[]) => number): void {
    const clientHooks = this.sdkHooks.get(client);
  ...
  public TriggerSDKHook(client: number, hookType: SDKHookType, ...args: unknown[]): number {
    const clientHooks = this.sdkHooks.get(client);
  ...
  public RegisterAPI(name: string, api: Record<string, unknown>, pluginName: string = "core"): void {
  ...
  public async Discord_SendMessage(channelId: string, content: string | object): Promise<boolean> {
