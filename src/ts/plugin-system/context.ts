...
import type { IGameBridge, CommandCallback, Timer, IMenu, ConVar, ClientCookie, SDKHookType } from "../shared/types/bridge";
...
import { Plugin_Stop, Plugin_Handled, ReplySource, Team } from "../shared/types/enums";
...
export class PluginContext implements IGameBridge {
  private listeners: Array<{ event: string; callback: (data: GameEvent) => void }> = [];
   ...
   */
  private menuCallbacks: Map<string, (client: number, info: string) => void> = new Map();
  private preListeners: Array<{ event: string; callback: (data: GameEvent) => number }> = [];
  private sdkHooks: Array<{ client: number; hookType: SDKHookType; callback: (...args: unknown[]) => number }> = [];
  ...
  constructor(
    ...
    const menuSelectHandler = (data: GameEvent) => {
      const anyData = data as unknown as { menuId: string; client: number; info: string };
  ...
  public CreateTimer(ms: number, callback: () => unknown, repeat?: boolean): Timer {
    let timer: Timer;
