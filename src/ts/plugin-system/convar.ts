...
export class ConVar implements IConVar {
	private changeHooks: Array<
		(cvar: IConVar, oldValue: string, newValue: string) => void
	> = [];
	...
	public AddChangeHook(
		callback: (cvar: IConVar, oldValue: string, newValue: string) => void,
	): void {
	...
	private SetValue(newValue: string): void {
		...
		if (this.onValueChange) {
			try {
				...
				console.error(
					`[ConVar] Error in onValueChange callback for ${this.name}:`,
					e,
				);
			}
		}
