 ...
 */
export function TeleportEntity(client: number, x: number, y: number, z: number): void {
 ...
 */
export function BanClient(steamId: string, reason: string, adminSteamId: string, duration: number, ip: string = ""): void {
 ...
 */
export function SetWeaponAmmo(client: number, weapon: string, ammo: number): void {
 ...
 */
export function SetEntityRenderColor(client: number, r: number, g: number, b: number, a: number): void {
 ...
 */
export function EmitSoundToClient(client: number, soundPath: string, volume: number = 1.0, channel: number = 0, pitch: number = 100): void {
 ...
 * @param soundPath Path to the sound file.
 */
export function EmitSoundToAll(soundPath: string, volume: number = 1.0, channel: number = 0, pitch: number = 100): void {
 ...
 */
export function ProcessTargetString(adminClient: number, targetPattern: string): number[] {
  ...
  if (targetPattern.startsWith("#")) {
    ...
    if (!userIdVal || isNaN(userIdVal)) return [];
 ...
 */
export function SetEntityVelocity(client: number, x: number, y: number, z: number): void {
  const p = GetContext().players.Get(client);
  if (p) {
    p.SetVelocity(x, y, z);
  }
 ...
 */
export function GetEntityVelocity(client: number): { x: number; y: number; z: number } {
  const p = GetContext().players.Get(client);
