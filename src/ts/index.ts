...
import type { BunSocket, BridgeProtocol } from "./shared/types/bridge";
import type { 
  GameEvent, 
  PlayerConnectEvent, 
              ...
              if (buffer.length >= 4 + length) {
                ...
                try {
                  ...
                  if (this.protocol === "length_prefixed_json") {
                    payload = JSON.parse(payloadBuffer.toString("utf-8")) as GameEvent;
                  } else {
  ...
  private HandlePayload(payload: GameEvent): void {
    ...
    if (payload.event === "PlayerConnect") {
      ...
      if (steam64 && steamApiKey) {
        fetch(`https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=${steamApiKey}&steamids=${steam64}`)
      ...
      this.pluginManager.emit("OnClientPostAdminCheck", { client: conn.client, player });
      ...
      if (player) {
        this.pluginManager.LogMessage(`Oyuncu ayrıldı: {Red}${player.name}{Default} (ID: ${disc.client})`);
        const logChannelId = process.env.DISCORD_LOG_CHANNEL_ID || this.settings.discord?.log_channel_id;
        if (logChannelId) {
          const discordPayload = {
      ...
      if (player) {
        ...
        player.UpdateObserverState(stats.isObserver ?? false, stats.observerTarget ?? 0);
