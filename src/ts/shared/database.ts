...
export class DatabaseManager {
  ...
  private LoadDriverConfig(): void {
    try {
      const configPath = join(process.cwd(), "configs", "core", "database.json");
  ...
  private Initialize(): void {
    ...
    try { this.db.run("ALTER TABLE players ADD COLUMN total_headshots INTEGER DEFAULT 0"); } catch (_) {}
    try { this.db.run("ALTER TABLE players ADD COLUMN total_damage INTEGER DEFAULT 0"); } catch (_) {}
    try { this.db.run("ALTER TABLE players ADD COLUMN total_mvps INTEGER DEFAULT 0"); } catch (_) {}
    try { this.db.run("ALTER TABLE players ADD COLUMN total_playtime INTEGER DEFAULT 0"); } catch (_) {}
    try { this.db.run("ALTER TABLE players ADD COLUMN is_muted INTEGER DEFAULT 0"); } catch (_) {}
    try { this.db.run("ALTER TABLE players ADD COLUMN is_gagged INTEGER DEFAULT 0"); } catch (_) {}
    try { this.db.run("ALTER TABLE admins ADD COLUMN expires_at INTEGER DEFAULT 0"); } catch (_) {}
    try { this.db.run("ALTER TABLE bans ADD COLUMN ip TEXT DEFAULT ''"); } catch (_) {}
  }
   ...
   */
  public AddDatabaseAdmin(steamid: string, flags: string, immunity: number = 0, groups: string = "", expires_at: number = 0): void {
    const query = this.db.prepare(`
   ...
   */
  public GetDatabaseAdmins(): Array<{ steamid: string; flags: string; immunity: number; groups: string; expires_at: number }> {
    const query = this.db.prepare("SELECT * FROM admins");
    return query.all() as Array<{ steamid: string; flags: string; immunity: number; groups: string; expires_at: number }>;
  }

  /**
   * Adds an administrative log entry to the database.
   */
  public AddAdminLog(adminSteamId: string, adminName: string, targetSteamId: string, targetName: string, action: string): void {
    const query = this.db.prepare(`
  ...
  public GetCookie(steamId: string, name: string): string {
    ...
    const row = query.get(steamId, name) as { cookie_value: string } | undefined;
