...
import { ReplySource, Buttons } from "./enums";
 ...
 */
export type BridgeProtocol = "ndjson" | "length_prefixed_json" | "length_prefixed_msgpack";
 ...
 */
export type CommandCallback = (client: number, args: string[]) => void | Promise<void>;
...
export interface IGameBridge {
  ...
  BanClient(steamId: string, reason: string, adminSteamId: string, duration: number, ip?: string): void;
  ...
  CreateMenu(title: string, callback: (client: number, info: string) => void): IMenu;
  ...
  SetEntityRenderColor(client: number, r: number, g: number, b: number, a: number): void;
  /** Emit a sound to a specific client. */
  EmitSoundToClient(client: number, soundPath: string, volume?: number, channel?: number, pitch?: number): void;
  /** Emit a sound to all connected clients. */
  EmitSoundToAll(soundPath: string, volume?: number, channel?: number, pitch?: number): void;
  ...
  CreateVote(question: string, options: string[], callback: (results: Record<string, number>) => void, durationMs?: number): void;
  ...
  CreateConVar(name: string, defaultValue: string, description?: string): ConVar;
  ...
  Discord_SendMessage(channelId: string, content: string | object): Promise<boolean>;
  ...
  SDKHook(client: number, hookType: SDKHookType, callback: (...args: unknown[]) => number): void;
  /** Unhooks a previously registered SDK callback. */
  SDKUnhook(client: number, hookType: SDKHookType, callback: (...args: unknown[]) => number): void;
...
export interface ConVar {
  ...
  AddChangeHook(callback: (cvar: ConVar, oldValue: string, newValue: string) => void): void;
}
