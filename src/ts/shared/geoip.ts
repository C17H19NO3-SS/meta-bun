	...
	private Initialize(): void {
		if (existsSync(this.dbPath)) {
			try {
				...
				const rawRanges = JSON.parse(content) as Array<{
					start: string;
					end: string;
					country: string;
				}>;
				this.ranges = rawRanges
					.map((r) => ({
				...
				console.error(
					"[GeoIP] Error loading database, falling back to defaults:",
					err,
				);
				this.LoadDefaults();
			}
	...
	public async UpdateDatabase(): Promise<boolean> {
		try {
			...
			const response = await fetch(
				"https://raw.githubusercontent.com/datasets/geoip2-ipv4/master/data/geoip-ranges.json",
			).catch(() => null);
			if (response?.ok) {
