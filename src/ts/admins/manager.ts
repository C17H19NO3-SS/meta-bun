...
export class AdminManager implements IAdminManager {
	...
	private groupsConfig: Map<
		string,
		{ flags: string; immunity: number; inherit?: string }
	> = new Map();
	...
	private LoadAdmins(): void {
		...
		try {
			...
			if (existsSync(configPath)) {
				const content = readFileSync(configPath, "utf-8");
				const admins = JSON.parse(content) as Record<
					string,
					| string
					| {
							flags?: string;
	...
	private resolveGroupImmunity(
		...
		if (group.inherit) {
			const inheritImm = this.resolveGroupImmunity(
				group.inherit,
				visitedGroups,
			);
	...
	public SetFlags(steamId: string, flags: string): void {
		...
		const currentImmunity =
			this.adminImmunity.get(steamId) ?? (flags.includes("z") ? 99 : 0);
