...
import type { PlayerDeathEvent, MapStartEvent } from "meta-bun/shared/types/events";
...
export default class SurfHPReward extends BasePlugin {
  ...
  constructor() {
    super();
    // Register ConVars
    this.hpRewardCvar = CreateConVar("sm_surf_hp_reward", "25", "HP amount granted on kill in surf maps.");
    this.maxHpCvar = CreateConVar("sm_surf_max_hp", "150", "Maximum HP a player can reach via rewards.");
  }
  ...
  public override OnPlayerDeath(data: PlayerDeathEvent): void {
    ...
    if (newHp > currentHp) {
      ...
      PrintToChat(attackerIndex, `{Green}[Surf]{Default} Adam vurduğun için {Lime}+${rewardAmount} HP{Default} kazandın!`);
    }
  }
  ...
  private CheckMap(): void {
    ...
    if (mapName.includes("surf_")) {
      this.isActive = true;
      console.log("[Surf HP Reward] Surf map detected. Plugin logic is now ACTIVE.");
    } else {
      this.isActive = false;
      console.log(`[Surf HP Reward] Non-surf map (${mapName}) detected. Plugin logic is INACTIVE.`);
    }
  }
}
