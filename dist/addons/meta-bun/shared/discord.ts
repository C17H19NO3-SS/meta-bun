...
export class DiscordService {
  ...
  private LoadConfig(): void {
    try {
      // Load Bot Token
      const settingsPath = join(process.cwd(), "configs", "core", "settings.json");
      ...
      // Load Permissions
      const permPath = join(process.cwd(), "configs", "core", "discord_permissions.json");
   ...
   */
  private HasPermission(pluginName: string, action: keyof DiscordPermissions, channelId?: string): boolean {
   ...
   */
  public async SendMessage(pluginName: string, channelId: string, content: string | object): Promise<boolean> {
    if (!this.token) {
